...
public class Stab extends SpecialAttack
{
	...
	public void onExecutionTick(EntityPlayer player, int progress)
	{
		...
		if (assumedToBeLookedAt != null)
		{
			...
			player.world.spawnParticle(EnumParticleTypes.CRIT, targetPos.xCoord + player.world.rand.nextDouble() / 2 - player.world.rand.nextDouble() / 2, targetPos.yCoord + assumedToBeLookedAt.getEyeHeight() + player.world.rand.nextDouble() / 2 - player.world.rand.nextDouble() / 2, targetPos.zCoord + player.world.rand.nextDouble() / 2 - player.world.rand.nextDouble() / 2, 0, 0, 0, new int[0]);
