...
import v0id.exp.block.BlockWaterWheel;

public class TileWaterWheel extends TileEntity implements ITickable, IMultiblock
{
    ...
    public void update()
    {
        if (this.world.getBlockState(this.pos).getBlock() instanceof BlockWaterWheel)
        {
            this.axis = this.world.getBlockState(pos).getValue(BlockHorizontal.FACING).getAxis();
        }

        int waterLevels = 0;
        BlockPos at = this.getPos().down(3);
        for (int i = -1; i <= 1; ++i)
        {
            BlockPos offset = at.offset(this.axis == EnumFacing.Axis.X ? EnumFacing.NORTH : EnumFacing.EAST, i);
            IBlockState state = this.getWorld().getBlockState(offset);
        ...
        float speed = 16 * waterLevels;
        float torque = 8 * waterLevels;
    ...
    {
        if (!breakingMultiblock && this.world.getBlockState(this.pos).getBlock() instanceof BlockWaterWheel)
        {
    ...
    {
        assert axis != EnumFacing.Axis.Y : "Water wheels can't be placed vertically!";
