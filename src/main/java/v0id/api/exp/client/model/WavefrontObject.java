package v0id.api.exp.client.model;
...
public class WavefrontObject
{
	...
	public void load(InputStream is)
	{
		...
		{
			ExPApi.apiLogger.error("Exception caught trying to read a model from an input stream!", ex);
		}
		finally
		{
	...
	public void load(String s)
	{
        ...
        for (String line : splitter.split(s))
        {
            ...
            if (line.startsWith("v "))
            {
                ...
                {
                    ExPApi.apiLogger.error("Model has invalid vertex definition at line %d! Vertices are specified in euclidean space and must contain 3 numbers separated by whitespace!", lines);
                }
            }
            ...
            if (line.startsWith("vn "))
            {
                ...
                {
                	ExPApi.apiLogger.error("Model has invalid normal definition at line %d! Vertex normals are euclidean vectors and must contain 3 numbers separated by whitespaces!", lines);
                }
            }
            ...
            if (line.startsWith("vt "))
            {
                ...
                {
                	ExPApi.apiLogger.error("Model has invalid texture coordinates definition at line %d! Texture coordinates are bi-dimensional space vectors and must contain 2 numbers separated by whitespaces!", lines);
                }
            }
            ...
            if (line.startsWith("f "))
            {
                ...
                {
                	ExPApi.apiLogger.error("Model has invalid face definition at line %d! Faces are pointers to vertexes, uvs and normals. They must contain 3 sequence of 3 numbers separated by / separated by whitespaces(ex.: 3/3/3 1/3/3 5/5/9)! Before exporting your model make sure to split all it's faces into triangles!", lines);
                }
            }
        }
        ...
        tempNormals.clear();
        tempFaces.clear();
        ExPApi.apiLogger.debug("Wavefront object model loaded!");
	}
